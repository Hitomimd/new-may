module.exports = (m) => {
   ...
   if (setting) {
      ...
      if (!('sk_author' in setting)) setting.sk_author = '@Ova'
      ...
      if (!('owners' in setting)) setting.owners = ['6285692242216']
      if (!isNumber(setting.lastReset)) setting.lastReset = new Date * 1
      if (!('msg' in setting)) setting.msg = 'Hi +tag 🪸\nI am an automated system (WhatsApp Bot) that can help to do something,\n\nIf you find an error or want to upgrade premium plan contact the owner.'
      if (!isNumber(setting.menuStyle)) setting.menuStyle = 4
      if (!('cover' in setting)) setting.cover = 'https://srv.neoxr.tk/files/Ff86r7.jpg'
      if (!('link' in setting)) setting.link = 'https://chat.whatsapp.com/KqGx272nHhV6lW1qxLmNOp'
   } else {
      global.db.setting = {
         autodownload: true,
         chatbot: true,
